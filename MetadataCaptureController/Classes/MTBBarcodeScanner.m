...
@property (nonatomic, strong) dispatch_semaphore_t resultSemaphore;
...
- (instancetype)initWithPreviewView:(UIView *)previewView
{
	...
	if (self)
	{
		...
		_resultSemaphore = dispatch_semaphore_create(0);
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
	...
	dispatch_async(dispatch_get_main_queue(), ^{
		...
		dispatch_semaphore_signal(_resultSemaphore);
	});

	dispatch_semaphore_wait(_resultSemaphore, DISPATCH_TIME_FOREVER);
}
...
- (AVCaptureSession *)newSessionWithCaptureDevice:(AVCaptureDevice *)captureDevice
{
	...
	[self.captureOutput setMetadataObjectsDelegate:self queue:dispatch_queue_create("CaptureQueue", DISPATCH_QUEUE_SERIAL)];
